...
export default function EmailModal({ isOpen, onClose,onUseMobile,onOpenOTP,formData, setFormData,onOpenLogin }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const payload = formData.phone ? formData : { email: formData.email };

            const response = await axios.post(`https://travomile-backend.vercel.app/formauth/sendotp`, payload, {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                ...
                <div className="flex justify-between text-sm text-gray-600 mb-4">
                    ...
                    <div>
                        <a href="#" className="hover:underline" onClick={onOpenLogin}>Use Password Login</a>
                    </div>
                </div>
